...
function midpoint(a: any, b: any, digits: any): any {
    if (b !== null && a >= b) {
        throw new Error(a + ' >= ' + b);
    }
...
function getIntegerLength(head: any) {
    if (head >= 'a' && head <= 'z') {
...
function validateInteger(int: any) {
...
function incrementInteger(x: any, digits: any) {
    validateInteger(x);
...
function decrementInteger(x: any, digits: any) {
    validateInteger(x);
    const [head, ...digs] = x.split('');
    let borrow = true;
...
// digits is a string such as '0123456789' for base 10.  Digits must be in
// ascending character code order!
export function generateKeyBetween(a: string, b: string, digits: string) {
    if (a !== null) {
        validateOrderKey(a);
    }
    if (b !== null) {
